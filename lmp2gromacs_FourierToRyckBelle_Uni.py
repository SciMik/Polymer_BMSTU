...
def parse_lammps_data(file_name, rPy_filename):
    print("Start parses")
    with open(rPy_filename) as f:
        lines_rPy = f.readlines()
    for i, line in enumerate(lines_rPy):
        if "Masses" in line:
            masses_start = i + 2
    masses = {}
    atom_type_name = {}
    for line in lines_rPy[masses_start:]:
        if not line.strip():
            break
    ...
    for line in lines[dihedral_coeffs_start:]:
        ...
        dihedral_coeffs_num = int(fields[1])
        coeffs = dict()
        for i in range(4):
            coeffs.update({i + 1: float(0.0)})
        # if len(coeffs) > 4:
        # print("Error+++++")
        # break
        coeffs_RB = dict()
        ...
        for i in range(dihedral_coeffs_num):
            num_c = int(fields[1 + 2 + i * 3])
            # print(coeffs[num_c])
            coeffs[num_c] = float(fields[1 + 1 + i * 3])
        if len(coeffs) > 4:
            print("Error-=-=-")
            break
...
def write_gro(data, gro_file):
    with open(gro_file, "w") as f:
        ...
        for atom in data["atoms"]:
            ...
            #if mol_id == 1:
            f.write("%5d%-5s%5s%5d%8.3f%8.3f%8.3f%8.4f%8.4f%8.4f\n" % (mol_id, "UNL", element, atom_id, x / 10, y / 10, z / 10, vx * 100, vy * 100, vz * 100))
                #f.write("%5d%-5s%5s%5d%8.3f%8.3f%8.3f%8.4f%8.4f%8.4f\n" % (mol_id, "MOL", element, atom_id, x / 10, y / 10, z / 10, vx * 100, vy * 100, vz * 100))
                #f.write("%5d%-5s%5s%5d%8.3f%8.3f%8.3f%8.4f%8.4f%8.4f\n" % (mol_id, "MOL", hex(el_counter[element][mol_id - 1] + 1)[2:] + element, atom_id, x/10, y/10, z/10, vx*100, vy*100, vz*100))
                #el_counter[element][mol_id - 1] += 1
    ...
    with open(top_file, "w") as f:
        ...
        for atom in data["atoms"]:
            ...
            if mol_id == 1:
                ...
                f.write(f"{atom_id:6d}  {element:<10s}{mol_id:6d}   UNL    {element:<6s}  {atom_id:6d}  {charge:10.20f} {data['masses'][atom_type]:10.6f} ; qtot {charge:10.20f}\n")
...
if __name__ == "__main__":
    ...
    data = parse_lammps_data(lammps_file, rPy_file)
    write_gro(data, gro_file)
    write_top(data, top_file)